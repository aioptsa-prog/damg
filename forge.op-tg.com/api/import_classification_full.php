...
// Load built-in full taxonomy JSON
$path = __DIR__ . '/../assets/classification_full.json';
if(!is_file($path)){
  http_response_code(500); echo json_encode(['error'=>'missing_asset']); exit;
}
$json = @file_get_contents($path);
$data = json_decode($json, true);
if(!$data || !is_array($data)){
  http_response_code(500); echo json_encode(['error'=>'invalid_asset']); exit;
}
...
try{
  ...
  $insKw = $pdo->prepare("INSERT INTO category_keywords(category_id, keyword, created_at) VALUES(?,?,datetime('now'))");
    foreach($data['keywords'] as $k){
      ...
      $cname = null; if(isset($k['category'])) $cname = trim((string)$k['category']); if(!$cname && isset($k['category_name'])) $cname = trim((string)$k['category_name']);
      if(!$cname && isset($k['category_id'])){
        if(!empty($data['categories']) && is_array($data['categories'])){
          foreach($data['categories'] as $c){ if(($c['id']??null)===($k['category_id']??null) && !empty($c['name'])){ $cname = $c['name']; break; } }
        }
      }
      $cid = $cname ? ($mapNameToId[$cname] ?? null) : null; if(!$cid) continue;
  ...
  $insRule = $pdo->prepare("INSERT INTO category_rules(category_id, target, pattern, match_mode, weight, note, enabled, created_at) VALUES(?,?,?,?,?,?,?,datetime('now'))");
    foreach($data['rules'] as $r){
      ...
      $cname = null; if(isset($r['category'])) $cname = trim((string)$r['category']); if(!$cname && isset($r['category_name'])) $cname = trim((string)$r['category_name']);
      if(!$cname && isset($r['category_id'])){
        if(!empty($data['categories']) && is_array($data['categories'])){
          foreach($data['categories'] as $c){ if(($c['id']??null)===($r['category_id']??null) && !empty($c['name'])){ $cname = $c['name']; break; } }
        }
      }
      $cid = $cname ? ($mapNameToId[$cname] ?? null) : null; if(!$cid || $pattern==='') continue;
  ...
  try{
    ...
    $who = (int)($u['id'] ?? 0);
    $line = sprintf("[%s] import_classification_full replace=%s cats_created=%d kw=%d rules=%d by_user=%d\n", date('c'), $replace?'1':'0', $catCreated, $kwInserted, $ruleInserted, $who);
